...
# @jonatasdp changes to use categories instead of tags
...
module Jekyll
  ...
  #     </div>
  #
  # The tag cloud itself is a collection of anchor categories, styled dynamically
  ...
  #
  # The plugin randomizes the order of categories every time the cloud is generated.
  class TagCloud < Liquid::Tag
    ...
    def render(context)
      ...
      weight.reduce("") do |html, tag|
        ...
        html << "<a style='font-size: #{size}#{unit}' href='/categories.html##{name}-ref'>#{name}</a>\n"
      end
    end
