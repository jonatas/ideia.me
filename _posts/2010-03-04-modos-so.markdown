...
{% mermaid %}
graph TD
    subgraph USpace [Modo Usuário - User Space]
        A[Aplicativos / Processos]
    end
    
    subgraph KSpace [Modo Kernel - Kernel Space]
        B((Interface de System Calls))
        C[Gerenciador de Processos]
        D[Gerenciador de Memória]
        E[Sistemas de Arquivos]
        F[Drivers de Dispositivos]
...
#### Qual é a principal diferença de privilégios entre o Modo Usuário e o Modo Kernel?

* O Modo Kernel tem acesso completo ao hardware, enquanto o Modo Usuário acessa recursos de forma restrita <- (Correto! Esta separação é vital para a segurança do SO)
* O Modo Usuário é usado apenas por administradores de rede, o Kernel por usuários comuns (Incorreto, os nomes não se referem aos tipos de conta de usuário)
* Não há diferença de privilégio, apenas uma variação na velocidade de processamento (Incorreto, o nível de privilégio é a característica que os define)
...
#### Como o sistema operacional reage se um comando no modo usuário tenta violar a segurança ou acessar locais proibidos?

* Executa o comando em modo de segurança e depois notifica o usuário (A verificação barra a execução antes mesmo de iniciar)
* O Kernel cancela a operação silenciosamente para não travar o processo (Sempre é retornado um código de erro ou exceção)
* O sistema envia uma exceção e nega a execução da operação <- (Exatamente! O Kernel protege o sistema e devolve o erro para a aplicação tratar)
