...
**Crucial Directive: All apps in this repository should follow same as the Yoga App.**
They must adhere to the same Core Principles and Technical Stack & Constraints defined below.
...
- **JavaScript Structure**:
  ...
  - When embedding inline JavaScript within Jekyll Markdown files, ensure backticks are not escaped (e.g., avoid `\``) to prevent browser syntax errors; use standard backticks (`` ` ``).
...
- **Frontend / Visual Verification**: Any frontend UI changes (HTML/CSS/JS) MUST be visually verified by starting a local server, writing a temporary Playwright script (`playwright.sync_api` or playwright node module) to capture a screenshot, and confirming it looks correct before finalizing.
  ...
  - When executing Playwright scripts to visually verify frontend UI changes, run a local Python HTTP server in the built `_site` directory using the command: `cd _site && python3 -m http.server 8000 &> /dev/null &`.
  - When using a local `python3 -m http.server` to verify Jekyll build output in the `_site` directory, ensure Playwright scripts navigate to explicit `.html` file paths (e.g., `http://127.0.0.1:8000/post-title.html`). Verify the exact generated path using `find _site -name '*.html'` beforehand, as permalink structures may differ from source directories and extensionless URLs are not automatically resolved.
- **Accessibility Directive (a11y)**:
  ...
  - Custom interactive elements (like SVGs or DIVs) acting as buttons must explicitly implement `keydown` event listeners for the 'Enter' and 'Space' keys, in addition to using `tabindex="0"` and `role="button"`, to ensure full keyboard accessibility.
  ...
  - To guarantee full accessibility in interactive state simulators (like toggle views), always use `aria-pressed` on toggle buttons and accompany visual state changes with explicit text updates in an `aria-live='polite'` region to communicate context changes to screen readers; do not rely purely on visual transitions to communicate state changes to screen readers.
...
- **Plan Constraints**: The pre-commit plan step must use the exact wording: 'Complete pre-commit steps to ensure proper testing, verification, review, and reflection are done.'
- **Guidelines regarding Memory Constraints**:
